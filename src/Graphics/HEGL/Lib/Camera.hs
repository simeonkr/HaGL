module Graphics.HEGL.Lib.Camera (
    rotatingView,
    interactiveView
) where

import Graphics.HEGL
import Graphics.HEGL.Lib.Math (idMat, rotate, translate, scale)
...
interactiveView initialEye = inverse curCam where
    ...
    camX = normalize . xyz_ $ col1 prevCam
    camY = normalize . xyz_ $ col2 prevCam

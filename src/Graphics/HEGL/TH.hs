...
-- e.g.: xy_ v = makeGL () $ OpCoordMulti (CoordX `CoordCons` (CoordY `CoordCons` CoordNil)) v
    ...
    gen coords@[x, y] = 
        FunD
            ...
            [Clause
                ...
                (NormalB
                (InfixE
                    (Just (AppE (VarE $ mkName "makeGL") (TupE [])))
                    (VarE $ mkName "$")
                    (Just
                        (AppE
                            (AppE
...
-- e.g.: xyz_ v = makeGL () $ OpCoordMulti (CoordX `CoordCons` (CoordY `CoordCons` (CoordZ `CoordCons` CoordNil))) v

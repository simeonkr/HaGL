...
module Graphics.HEGL.Numerical (
    ...
    fromList,
    eltAt, eltsAt, matCol,
    Graphics.HEGL.Numerical.length,
    distance,
    dot,
    cross,
    normalize,
...
import Graphics.HEGL.Util.Types (FinList, flToList)
import Graphics.HEGL.Util (warn)
...
normalize v@(Mat xs) = 
    Mat $ listArray (bounds xs) (map (/ Graphics.HEGL.Numerical.length v) (elems xs))
